...
app.controller('empleadoController', ['$scope', '$http', 'fileUpload', function($scope, $http, fileUpload){
    ...
    $scope.entregable = function() {
        ...
        $http.put(url_server+"tarea/actualizar", tarea).success(function(response) {
            ...
            var data = {
                ...
                message: "Se ha enviado el entregable",
                timeout: 4000
            };
            notification.MaterialSnackbar.showSnackbar(data);
    ...
    $scope.iniciar = function(){
        //alert("iniciar edit "+edit+" $scope.tarea "+$scope.tarea._id+" $scope.TARSUP "+$scope.tarea.TARSUP);
        var tarea = $scope.tarea
        tarea.id = tarea._id; // Pasamos la _id a id para mayor comodidad del lado del servidor a manejar el dato.
        delete tarea._id
        tarea.TARSTA = 'P';
    ...
    socket.on("aviso_fin_tareas", function (idOperador,mensaje) {
        ...
        var user = JSON.parse(usuario)
        var myName = user._id;
        //alert("idOperador "+idOperador+" myName "+myName)
        if (myName == idOperador) {
    ...
    socket.on("aviso_validar_tarea", function (idOperador,mensaje) {
        playBeep()
        vibrate()
        var user = JSON.parse(usuario)
        var myName = user._id;
    ...
    socket.on("aviso_rechazar_tarea", function (idOperador,mensaje) {
        playBeep()
        vibrate()
        var user = JSON.parse(usuario)
        var myName = user._id;
